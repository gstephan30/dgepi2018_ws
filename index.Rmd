...
> 5. Denke wie ein Designer - "weniger ist of mehr"
...
p <- plot_ly(
  ...
  node = list(
    label = nodes,
    color = sample("blue", length(df[[1]]), replace = TRUE),
    pad = 15,
    thickness = 20,
    line = list(
      color = "blue",
...
g <- g +
  ylab(" ") + 
  xlab(" ") +
  ggtitle("Number of Foodborne Cases of selected Species\nin the USA between 1998 and 2015")
g
```
---
# 3. Säubere die Darstellung 
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
g2 <- read_csv("data/outbreaks.csv") %>% 
  group_by(State) %>% 
...
df2 %>% 
  add_count(Illnesses) %>% 
  select(Month, Illnesses, n) %>% 
  mutate(Month = month(ymd(paste0("2000-", Month, "-1")), label = TRUE)) %>% 
  group_by(Month, Illnesses) %>% 
  summarise(n_month = n()) %>% 
  ungroup() %>% 
  #filter(Illnesses == 20) %>% 
  left_join(df2 %>% count(Illnesses)) %>% 
  mutate(per = n_month/n*100) %>% 
  filter(n>=500) %>% 
  select(Month, Illnesses, per) %>% 
  mutate(Illnesses = as.character(Illnesses)) %>% 
  spread(Illnesses, per) %>% 
  ggplot(aes(y = Month, x = per, fill = per)) +
  #geom_area() +
